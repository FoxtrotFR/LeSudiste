...
def move(players,gravite) :
    ...
    if players.plateforme==0 and players.memoireup==0 and gravite==1: #appliquer la gravité (0 pas sur palteforme)
        up_down(players,1) 
    elif players.memoireup!=0 and gravite == 1: #faire le saut du player
        up_down(players,-1)
        players.memoireup-=1
    elif players.right !=0  and gravite == 2: #bouger le players vers la droite si la touche est appuyée
        right_left(players,1)
        players.right -=1
    elif players.left !=0  and gravite == 2:  #bouger le players vers la gauche si la touche est appuyée
        right_left (players,-1)
        players.left -=1
    return players
